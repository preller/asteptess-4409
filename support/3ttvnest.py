...
def fit_and_plot_sine_wave_with_smooth_model(period_range, amplitude_range, period_step, amplitude_step, save_path=None, show_plot=True):
    best_loss = np.inf
    ...
    # Using the best parameters to generate a smooth sine wave over a dense range of x values
    best_period, best_amplitude, best_phase_shift = best_params
    dense_x = np.linspace(midtimes.min(), midtimes.max(),
                          1000)  # Dense range for smooth plot
    smooth_y_fit = generate_sine_wave(
        dense_x, best_period, best_amplitude, best_phase_shift)
    ...
    if show_plot or save_path:
        ...
        plt.plot(dense_x, smooth_y_fit, 'r-', label='Smooth Model: Period = ' + str(best_period) +
                 ' days, Amplitude = ' + str(round(best_amplitude, 2)) + ' minutes')
        plt.axhline(y=0, color='r', linestyle='--')
        plt.title(
            'TTV Data Over Time (in Minutes) with Smooth Sine Wave Fit (BJD-2458000)')
...
# Running the adjusted function
fit_and_plot_sine_wave_with_smooth_model(
    period_range=(500, 600),
    amplitude_range=(5, 15),
    period_step=10,
    amplitude_step=0.1,
    save_path='./plots/ttv_sine_fit4.png',
    show_plot=False
)
