...
def fit_and_plot_sine_wave(period_range, amplitude_range, period_step, amplitude_step, save_path=None, show_plot=False):
    best_loss = np.inf
    best_params = (None, None, None)  # (period, amplitude, phase_shift)
    x = midtimes
    y = ttv_median_minutes
        ...
        for amplitude in np.arange(amplitude_range[0], amplitude_range[1] + amplitude_step, amplitude_step):
            for phase_shift in np.linspace(0, 2 * np.pi, 100):
                fitted_y = generate_sine_wave(
                    x, period, amplitude, phase_shift)
                loss = np.mean((y - fitted_y) ** 2)
                if loss < best_loss:
                    best_loss = loss
                    best_params = (period, amplitude, phase_shift)

    best_period, best_amplitude, best_phase_shift = best_params
    y_fit = generate_sine_wave(
        x, best_period, best_amplitude, best_phase_shift)

    if show_plot or save_path:
        plt.figure(figsize=(12, 6))
        plt.errorbar(x, y, yerr=[lower_error_minutes, upper_error_minutes], fmt='o',
                     ecolor='lightgray', elinewidth=3, capsize=0, label='TTV Data with Errors')
        plt.plot(x, y_fit, 'r-', label='Best Fit: Period = ' + str(best_period) +
                 ' days, Amplitude = ' + str(round(best_amplitude, 2)) + ' minutes')
        plt.axhline(y=0, color='r', linestyle='--')
        plt.title(
            'TTV Data Over Time (in Minutes) with Best Fit Sine Wave (BJD-2458000)')
...
# # Example usage
# best_params = fit_and_plot_sine_wave(
#     period_range=(300, 500),  # Period range in days
#     amplitude_range=(8, 12),  # Amplitude range in minutes
#     period_step=10,  # Period step in days
#     amplitude_step=0.1,  # Amplitude step in minutes
#     save_path=None,  # Specify a path to save the plot or None to not save
#     show_plot=True  # Set to True to display the plot or False to not display
# )

# print("Best Fit Parameters: Period = ", best_params[0], "days, Amplitude = ", round(
#     best_params[1], 2), "minutes, Phase Shift = ", best_params[2])

# Run again, save the plot
best_params = fit_and_plot_sine_wave(
    period_range=(300, 500),
    amplitude_range=(8, 12),
    period_step=10,
    amplitude_step=0.1,
    save_path='./plots/ttv_sine_fit.png',
    show_plot=False
)
